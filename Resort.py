...
def screen1():
    ...
    print("""
            ***************HOTEL MANAGEMENT SYSTEM *************************
            #******* Designed and Maintained By: Agrasth***************************
            """)
    print("""
                1--->NEW LOGIN
                2--->OLD
...
def bookingRecord():
    ...
    if customer:
        myConnection=mysql.connector.connect(host="localhost",user=userName,passwd=password ,database='HMS',auth_plugin='mysql_native_password' )
        cursor=myConnection.cursor()
        createTable ="CREATE TABLE IF NOT EXISTS BOOKING_RECORD(CID VARCHAR(20),CHECK_INDATE DATE,CHECK_OUT DATE)"
        cursor.execute(createTable)
...
def searchCustomer():
    global cid
    myConnection=mysql.connector.connect(host="localhost",user='root',passwd=password ,database='HMS',auth_plugin='mysql_native_password' )
    cursor=myConnection.cursor()
